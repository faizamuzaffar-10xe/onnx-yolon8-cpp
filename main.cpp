...
//     cv::Mat final_concat2 = concat_along_axis3(x11_conv, x_sigmoid, x_concat1_reshaped,1);
...
//     std::cout << "out.dims: " << final_concat2.size << std::endl;


//     std::vector<float> final_concat;
//     final_concat.assign((float*)final_concat2.datastart, (float*)final_concat2.dataend);
...
//             output[c][i] = final_concat[c * num_preds + i];
//         }
//     }
...
std::vector<float> preprocess_frame(const cv::Mat& frame, const cv::Size& input_size, cv::Mat& out_img) {
    cv::Mat img;
...
int main() {
    ...
    cv::VideoWriter writer("output_video.mp4", cv::VideoWriter::fourcc('m','p','4','v'), fps, {frame_width, frame_height});

    cv::Mat frame;
    while (cap.read(frame)) {
        // Preprocess
        cv::Mat image_for_draw;
        std::vector<float> input_tensor_values = preprocess_frame(frame, input_size, image_for_draw);
        Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
            memory_info, input_tensor_values.data(), input_tensor_values.size(),
            input_shape.data(), input_shape.size());
        ...
        for (int idx : nms_indices) {
            ...
            std::ostringstream oss;
            oss << "Cls: " << cls_id << " " << class_scores[idx][0];  // class 0 = person
    ...
    cap.release();
    writer.release();
    std::cout << "Saved video to output_video.mp4\n";
    return 0;
}
